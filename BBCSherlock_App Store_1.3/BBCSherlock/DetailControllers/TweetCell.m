...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        self.backgroundView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"tweetcell_background.png"]];
    }
    return self;
}
...
-(void) adjustWithString:(NSString*)content withImage:(NSString*) url andDate:(NSString*) date andAvatar:(NSString*)profileUrl andName:(NSString*)name
{
    ...
    if(name && [name isKindOfClass:[NSString class]] && ![name isEqualToString:@""])
    {
        ...
        [self.weiboLabel setFrame:CGRectMake(MARGIN_TO_LEFT+PROFILE_WIDTH+PADDING, MARGIN_TO_TOP + 25, labelsize.width, labelsize.height*1.3)];   //此处weiboLabel的frame左侧为头像预留，上部加上nameLabel的高度
    } 
    else
    {
        [self.weiboLabel setFrame:CGRectMake(MARGIN_TO_LEFT+PROFILE_WIDTH+PADDING, MARGIN_TO_TOP, labelsize.width, labelsize.height*1.3)];   //此处weiboLabel的frame左侧为头像预留，上部不加上nameLabel的高度
    }
    ...
    ///////头像图片
    if(profileUrl && [profileUrl isKindOfClass:[NSString class]] && ![profileUrl isEqualToString:@""])
    {
        ...
        [self.avatarImage setBackgroundColor:[UIColor clearColor]];
    }
    else
    {
        [self.weiboLabel setFrame:CGRectMake(MARGIN_TO_LEFT+PADDING, MARGIN_TO_TOP, labelsize.width, labelsize.height*1.3)];  
        //没有头像图片，修正weiboLabel的frame左侧坐标
    }
...
{    
    ...
    NSString *profileurl = [dict objectForKey:kAvatarTag];
    NSString *name = [dict objectForKey:kNameTag];
    NSString *date  = [dict objectForKey:kTimeTag];
    NSString *image = [dict objectForKey:kTweetPicTag];
    
    [self adjustWithString:text withImage:image andDate:date andAvatar:profileurl andName:name];
}
